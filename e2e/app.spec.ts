...
test('the Venus route is a complete, attributed field trip with one live viewport', async ({ page }) => {
  await page.goto('/venus')

  await expect(page).toHaveURL(/\/venus$/)
  await expect(page.getByRole('heading', { name: 'Venus, beneath the veil' })).toBeVisible()
  await expect(page.locator('canvas')).toHaveCount(1)
  await expect(page.locator('canvas')).toBeVisible()
  await awaitGeometry(page)

  // Every beat exists in the accessibility tree without advancing a carousel.
  ...
  // The static fallback is present beneath the live canvas from first paint.
  await expect(page.getByTestId('venus-poster')).toBeAttached()
  await expect(page.getByTestId('venus-poster').locator('img')).toHaveAttribute(
    'src',
    /mariner-clouds/,
  )

  await expect(page.getByText('92 bar', { exact: true }).last()).toBeAttached()
  await expect(page.getByText('464 °C', { exact: true }).last()).toBeAttached()
  await expect(page.getByText(/CO₂ · 96.5%/)).toBeAttached()
  await expect(page.getByText(/does not prove one by itself/)).toBeAttached()
  await expect(page.getByText(/at least 85%/)).toBeAttached()
  await expect(page.getByText(/Brightness records radar return/).first()).toBeAttached()
  await expect(page.getByRole('link', { name: 'USGS Magellan global mosaic' })).toHaveAttribute(
    'href',
    /astrogeology\.usgs\.gov/,
  )
})

test('Venus navigation preserves Visit and supports history plus the in-page back link', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('tab', { name: 'Systems' }).click()

  // Orbit view offers the lesson without changing any planet card's Visit action.
  await page.getByRole('button', { name: 'Orbit view' }).click()
  await expect(page.getByRole('link', { name: 'Explore Venus' })).toHaveAttribute('href', '/venus')
  ...
  const fieldTrip = page.getByRole('link', { name: 'Open the Venus field trip' })
  await expect(fieldTrip).toHaveAttribute('href', '/venus')

  // Reseeding detaches the sculpted result from canonical Venus, so the
  // measured field trip is no longer offered. Visiting Venus restores it.
  await page.getByLabel('Seed', { exact: true }).fill('23')
  await expect(page.getByRole('link', { name: 'Open the Venus field trip' })).toHaveCount(0)
  await page.getByRole('tab', { name: 'Systems' }).click()
  await page.getByRole('button', { name: /Venus/ }).click()
  await page.getByRole('link', { name: 'Open the Venus field trip' }).click()
  await expect(page).toHaveURL(/\/venus$/)
  ...
  await page.getByRole('link', { name: /Back to Little Worlds/ }).click()
  await expect(page).toHaveURL(/\/$/)
  await expect(page.getByRole('heading', { name: 'Peachmoss' })).toBeVisible()
})

test('the Venus viewport stops automatic rotation for reduced motion', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await page.goto('/venus')
  await expect(page.getByText('Rotation paused')).toBeVisible()
  await expect.poll(() => datum(page, 'triangles'), { timeout: 20_000 }).toBeGreaterThan(100)

  // Wait for async texture and shader work, then prove the engine becomes idle.
...
test('a WebGL failure reveals the Venus poster without hiding the lesson', async ({ page }) => {
  await page.goto('/venus')
  await expect(page.locator('canvas')).toBeVisible()
  ...
  await expect(page.getByTestId('venus-poster')).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Scan: Why Venus Is Hell' })).toHaveCount(1)
  await expect(page.getByRole('status')).toContainText('complete field trip')
})

test('world and system share routes still load after routing becomes reactive', async ({ page }) => {
  const params = {
    seed: 12345, preset: 'desert', mountains: 0.45, water: 0.12, roughness: 0.6,
    clouds: 0.18, glow: 0.4, ice: 0, lightAz: 0.107, lightEl: 0.639, spinDir: 1,
  ...
  await page.route('**/api/worlds/world-123', (route) => route.fulfill({
    json: {
      world: {
        slug: 'world-123', name: 'Shared Dune', params, dot: '#e0b070',
        sub: 'dune world', createdAt: '2026-07-29T00:00:00Z',
      },
    },
  }))
  await page.route('**/api/systems/system-123', (route) => route.fulfill({
    json: {
      system: {
        slug: 'system-123', name: 'Shared System', dot: '#ffb478', sub: '1 world',
        createdAt: '2026-07-29T00:00:00Z',
        def: {
          id: 'shared', name: 'Shared System', sub: 'a shared system', origin: 'custom',
          star: { name: 'Shared Star', color: 0xffb478, mass: 1 },
          bodies: [{
            name: 'Shared Dune', a: 1, period: 1, e: 0, inc: 0, node: 0, peri: 0,
            radius: 1, tilt: 0, flattening: 0, day: 24, params, texture: null, ring: null,
          }],
        },
      },
    },
  ...
  await page.goto('/w/world-123')
  await expect(page.getByRole('heading', { name: 'Shared Dune' })).toBeVisible()

  await page.goto('/s/system-123')
  await expect(page.getByRole('heading', { name: 'Shared System' })).toBeVisible()
