...
Visit a moon and the planet it belongs to is there with it, wheeling slowly around as the moon goes round — from the moon's own frame those are the same motion, so the relationship shows without the world you are sculpting ever leaving the centre. Every number is measured, so each planet looms exactly as large as it truly does from that moon: Jupiter fills 12° of Europa's sky, Saturn more than half fills Enceladus's, and Earth from our own Moon is the modest disc it really is. A tidally locked moon keeps one face toward its planet here too, because both take their angle from the same clock — and the time controls speed the pair together.
